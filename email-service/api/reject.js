...
export default async function handler(req, res) {
  ...
  if (approval.status !== 'pending') {
    res.setHeader('Content-Type', 'text/html');
    return res.status(400).send(page('Already Processed', `<p>This approval was already <strong>${approval.status}</strong>.</p>`));
  }

  await db.ref(`approvals/${approvalId}/status`).set('rejected');
  await db.ref(`approvals/${approvalId}/rejectedAt`).set(new Date().toISOString());

  const campaignId = approval.campaignId;
  if (campaignId) {
    await db.ref(`campaigns/${campaignId}/status`).set('cancelled');
    await db.ref(`campaigns/${campaignId}/cancelledAt`).set(new Date().toISOString());
  }

  res.setHeader('Content-Type', 'text/html');
  return res.status(200).send(page('Rejected',
    '<p>The campaign has been rejected and will not be sent.</p>'
  ));
}
