...
export default function PartnerPortal({ type, title, subtitle, onClose, user }) {
  ...
  const [application, setApplication] = useState({ name: "", website: "", description: "" });
  ...
  const load = async () => {
    ...
    try {
      ...
      if (mine?.approved) {
        const [savedCourses, savedTemplates] = await Promise.all([fetchPartnerCourses(mine.id), fetchAgencyTemplates(mine.id)]);
  ...
  const apply = async (event) => {
    ...
    await saveAgency({ ...application, partnerType: type, emails: [email], ownerEmail: email, approved: false });
  ...
  if (!user) return <main style={{ maxWidth: "760px", margin: "0 auto", padding: "7rem 1.5rem 2rem" }}><div style={card}><h1 style={{ marginTop: 0 }}>{title}</h1><p>Sign in to apply for a workspace and manage your program.</p><button className="btn-sharp" onClick={onClose}>Back</button></div></main>;
  if (loading) return <main style={{ padding: "7rem 1.5rem", textAlign: "center" }}>Loading workspace…</main>;

  if (!account) return <main style={{ maxWidth: "760px", margin: "0 auto", padding: "7rem 1.5rem 2rem" }}><header style={{ background: "#000", color: "#fff", padding: "1.5rem", marginBottom: "1.5rem" }}><h1 style={{ margin: 0, textTransform: "uppercase", fontSize: "1.35rem" }}>{title}</h1><p style={{ margin: "0.35rem 0 0", opacity: 0.8 }}>{subtitle}</p></header><form onSubmit={apply} style={card}><h2 style={{ marginTop: 0, fontSize: "1rem", textTransform: "uppercase" }}>Apply for a workspace</h2><p style={{ fontSize: "0.85rem", lineHeight: 1.5 }}>Your application is reviewed by DEV/CRAFT admin. Once approved, the main contact can create content and invite workspace admins.</p><label>Organization name<input required value={application.name} onChange={(e) => setApplication({ ...application, name: e.target.value })} style={input} /></label><label>Website<input value={application.website} onChange={(e) => setApplication({ ...application, website: e.target.value })} style={input} /></label><label>Program details<textarea required rows={4} value={application.description} onChange={(e) => setApplication({ ...application, description: e.target.value })} style={{ ...input, resize: "vertical" }} /></label><button className="btn-sharp" type="submit" style={{ marginTop: "1rem" }}>Submit application</button></form></main>;
  ...
  return <main style={{ maxWidth: "1080px", margin: "0 auto", padding: "6.5rem 1.5rem 2rem" }}>
    ...
    <nav aria-label="Workspace sections" style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.25rem" }}>{tabButton("overview", "Overview")}{tabButton("courses", type === "university" ? "Courses" : "Integrations")}{tabButton("templates", "Templates")}{tabButton("team", "Team")}</nav>
    ...
    {tab === "team" && <section style={card}><h2 style={{ marginTop: 0, fontSize: "1rem" }}>Workspace team</h2><p style={{ fontSize: "0.85rem" }}>Only the main contact can invite or remove administrators for this workspace.</p>{(account.emails || []).map((member) => <div key={member} style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #000", padding: "0.75rem 0" }}><span>{member}{member === account.ownerEmail && " (main contact)"}</span>{isOwner && member !== account.ownerEmail && <button onClick={async () => { await removeAgencyAdminEmail(account.id, member); load(); }} style={{ border: "0", background: "#000", color: "#fff", cursor: "pointer" }}>Remove</button>}</div>)}{isOwner && <form onSubmit={addAdmin} style={{ display: "flex", gap: "0.5rem", marginTop: "1rem", maxWidth: "520px" }}><input type="email" required placeholder="new-admin@example.com" value={newAdmin} onChange={(e) => setNewAdmin(e.target.value)} style={input} /><button className="btn-sharp" type="submit">Invite</button></form>}</section>}
  </main>;
}
