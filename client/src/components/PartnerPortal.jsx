...
export default function PartnerPortal({ type, title, subtitle, onClose, user }) {
  ...
  const [application, setApplication] = useState({ name: "", website: "", description: "", contactName: "", contactPhone: "", contactEmail: "" });
  ...
  if (!account) return <main style={{ maxWidth: "760px", margin: "0 auto", padding: "7rem 1.5rem 2rem" }}><header style={{ background: "#000", color: "#fff", padding: "1.5rem", marginBottom: "1.5rem" }}><h1 style={{ margin: 0, textTransform: "uppercase", fontSize: "1.35rem" }}>{title}</h1><p style={{ margin: "0.35rem 0 0", opacity: 0.8 }}>{subtitle}</p></header><form onSubmit={apply} style={card}><h2 style={{ marginTop: 0, fontSize: "1rem", textTransform: "uppercase" }}>Request access</h2><p style={{ fontSize: "0.85rem", lineHeight: 1.5 }}>All fields are reviewed by DEV/CRAFT admin before your workspace is approved.</p><label>Organization / university name<input required value={application.name} onChange={(e) => setApplication({ ...application, name: e.target.value })} style={input} /></label><label>Contact name<input required value={application.contactName} onChange={(e) => setApplication({ ...application, contactName: e.target.value })} style={input} /></label><label>Contact email<input required type="email" value={email} readOnly style={{ ...input, background: "#f2f2f2" }} /></label><label>Contact phone<input required type="tel" value={application.contactPhone} onChange={(e) => setApplication({ ...application, contactPhone: e.target.value })} style={input} /></label><label>Website / URL<input required type="url" value={application.website} onChange={(e) => setApplication({ ...application, website: e.target.value })} style={input} /></label><label>Program or integration requirements<textarea required rows={4} value={application.description} onChange={(e) => setApplication({ ...application, description: e.target.value })} style={{ ...input, resize: "vertical" }} /></label><button className="btn-sharp" type="submit" style={{ marginTop: "1rem" }}>Submit request</button></form></main>;
