...
export default function PartnerPortal({ type, title, subtitle, onClose, user }) {
  ...
  const publicDetails = <><header style={{ background: "#000", color: "#fff", padding: "2.25rem", marginBottom: "1.5rem", border: "2px solid #000", boxShadow: "5px 5px 0 #333" }}><div style={{ fontSize: "0.72rem", fontWeight: 800, letterSpacing: "1px" }}>DEV/CRAFT PARTNERS · INFORMATION PAGE</div><h1 style={{ margin: "0.5rem 0", textTransform: "uppercase" }}>{details?.title || title}</h1><p style={{ maxWidth: "720px", lineHeight: 1.6, opacity: 0.85 }}>{details?.description || subtitle}</p><p style={{ marginBottom: 0, fontSize: "0.82rem" }}>Status: {details?.status || "Available by approval"}</p></header><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "1rem" }}>{detailSections.map(([heading, content]) => <section key={heading} style={card}><h2 style={{ marginTop: 0, fontSize: "1rem", textTransform: "uppercase" }}>{heading}</h2><p style={{ whiteSpace: "pre-wrap", lineHeight: 1.6, marginBottom: 0 }}>{content}</p></section>)}</div>{details?.contact && <section style={{ ...card, marginTop: "1rem" }}><strong>Contact</strong><p style={{ marginBottom: 0 }}>{details.contact}</p></section>}<div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap", marginTop: "1.25rem" }}>{actionButtons.map((action, index) => <a key={`${action.label}-${index}`} className="btn-sharp" href={action.url || "#"}>{action.label}</a>)}</div></>;
