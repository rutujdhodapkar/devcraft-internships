        ...
        {activeTab === "receipt" && (
          <div style={{ maxWidth: "800px" }}>
            ...
            <p style={{ fontSize: "0.85rem", color: "#666", marginBottom: "1rem" }}>
              ...
              <code>{"{{receiptNo}}"}</code> <code>{"{{date}}"}</code> <code>{"{{status}}"}</code>{" "}
              <code>{"{{name}}"}</code> <code>{"{{email}}"}</code> <code>{"{{phone}}"}</code>{" "}
              <code>{"{{college}}"}</code> <code>{"{{city}}"}</code> <code>{"{{country}}"}</code>{" "}
              <code>{"{{domain}}"}</code> <code>{"{{duration}}"}</code> <code>{"{{amount}}"}</code>{" "}
              <code>{"{{paidAmount}}"}</code> <code>{"{{paymentCurrency}}"}</code>{" "}
              <code>{"{{paymentMethod}}"}</code> <code>{"{{paymentGateway}}"}</code>{" "}
              <code>{"{{transactionId}}"}</code> <code>{"{{paidAt}}"}</code>{" "}
              <code>{"{{internId}}"}</code> <code>{"{{upiId}}"}</code> <code>{"{{referralCode}}"}</code>.
...
<style>body{font-family:monospace;padding:2rem;max-width:650px;margin:0 auto;color:#000}
h2{font-size:1rem;border-bottom:1px solid #ccc;padding-bottom:0.3rem;margin-top:1.2rem}
...
<p style="font-size:0.7rem;color:#888;margin-top:1rem;text-align:center">Generated by Admin Panel</p>
                      ...
                      const paidAtFormatted = r.paidAt ? new Date(r.paidAt).toLocaleString() : "";
                      const vars = {
                        ...
                        phone: r.phone || selectedIntern.phone || "",
                        college: r.college || selectedIntern.college || "",
                        city: r.city || selectedIntern.city || "",
                        country: r.country || selectedIntern.country || "",
                        domain: r.domain || selectedIntern.domain || "",
                        duration: r.duration || selectedIntern.duration || "",
                        amount: "\u20B9" + (r.amount || selectedIntern.paymentAmount || 0),
                        paidAmount: r.paidAmount ? "\u20B9" + r.paidAmount : "\u20B9" + (r.amount || selectedIntern.paymentAmount || 0),
                        paymentCurrency: r.paymentCurrency || "INR",
                        paymentMethod: r.paymentMethod || "Online",
                        paymentGateway: r.paymentGateway || "Dodo",
                        ...
                        internId: r.internId || selectedIntern.internId || "",
                        upiId: r.upiId || selectedIntern.upiId || "",
                        referralCode: r.referralCode || selectedIntern.referralCode || "",
    ...
    <div style={{ border: "2px solid #000", padding: "1.25rem", boxShadow: "4px 4px 0 #000" }}>
      ...
      {editAgency && (
        <div style={{ border: "2px solid #000", padding: "1rem", marginBottom: "1rem", background: "#f0f0f0" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", marginBottom: "0.5rem" }}>
            <div><label style={{ fontSize: "0.65rem", fontWeight: 700, display: "block" }}>Features (one per line)</label><textarea value={editAgency.features || ""} onChange={e => setEditAgency(p => ({ ...p, features: e.target.value }))} rows={3} style={{ ...is, resize: "vertical" }} /></div>
            <div><label style={{ fontSize: "0.65rem", fontWeight: 700, display: "block" }}>Use cases (one per line)</label><textarea value={editAgency.useCases || ""} onChange={e => setEditAgency(p => ({ ...p, useCases: e.target.value }))} rows={3} style={{ ...is, resize: "vertical" }} /></div>
            <div><label style={{ fontSize: "0.65rem", fontWeight: 700, display: "block" }}>Documentation / resources</label><textarea value={editAgency.resources || ""} onChange={e => setEditAgency(p => ({ ...p, resources: e.target.value }))} rows={3} style={{ ...is, resize: "vertical" }} /></div>
            <div><label style={{ fontSize: "0.65rem", fontWeight: 700, display: "block" }}>Requirements / approval rules</label><textarea value={editAgency.requirements || ""} onChange={e => setEditAgency(p => ({ ...p, requirements: e.target.value }))} rows={3} style={{ ...is, resize: "vertical" }} /></div>
            <div><label style={{ fontSize: "0.65rem", fontWeight: 700, display: "block" }}>Pricing / terms / FAQ</label><textarea value={editAgency.termsFaq || ""} onChange={e => setEditAgency(p => ({ ...p, termsFaq: e.target.value }))} rows={3} style={{ ...is, resize: "vertical" }} /></div>
            <div><label style={{ fontSize: "0.65rem", fontWeight: 700, display: "block" }}>Enabled actions (comma separated)</label><input value={editAgency.enabledActions || "Apply,Contact,Visit Website"} onChange={e => setEditAgency(p => ({ ...p, enabledActions: e.target.value }))} style={is} /><small>Apply, Request API Key, Documentation, Contact, Join Waitlist, External Link, Email, Call, WhatsApp</small></div>
          </div>
          <div style={{ marginBottom: "0.5rem" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", border: "2px solid #000", padding: "0.75rem 1rem", background: "#fafafa" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flex: 1 }}>
                ...
                <div>
                  ...
                  {a.requestedHooks && <div style={{ fontSize: "0.72rem", color: "#666" }}>Requested MCP: {a.requestedHooks}</div>}
...
function McpApiSection({ user }) {
  ...
  const blank = { title: "", description: "", status: "Available by approval", html: "", features: "", useCases: "", resources: "", requirements: "", approvalRequirements: "", pricing: "", terms: "", faq: "", contact: "", actions: [] };
...
Configure in ChatGPT:
  URL: https://devcraft.fennark.xyz/api/mcp-domains`}
      ...
      <div style={{ border: "2px solid #000", padding: "1.25rem", boxShadow: "4px 4px 0 #000" }}>
        ...
        <label style={{ fontSize: "0.75rem", fontWeight: 700, display: "block", marginTop: "1rem" }}>Custom HTML (shown at top of the public page)
          <textarea rows={10} value={details.html || ""} onChange={e => update("html", e.target.value)} placeholder={`<h2>Welcome</h2>\n<p>Any HTML here is shown on the page.</p>\n<button data-action="request">Request access</button>\n<a data-action="link" data-url="https://docs.example.com">Read docs</a>`} style={{ width: "100%", boxSizing: "border-box", border: "1px solid #000", padding: "0.45rem", marginTop: "0.2rem", fontFamily: "monospace", fontSize: "0.78rem" }} />
        </label>
        <p style={{ fontSize: "0.72rem", color: "#666", marginTop: "0.35rem" }}>Add functional buttons inside HTML with <code>data-action</code>: <code>request</code> (opens the request form), <code>back</code> (returns home), or <code>link</code> with a <code>data-url</code>.</p>
        <div style={{ marginTop: "1rem" }}><strong style={{ fontSize: "0.8rem" }}>Action buttons</strong>{details.actions.map((action, index) => <div key={index} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 2fr auto auto", gap: "0.4rem", marginTop: "0.4rem" }}><input placeholder="Label" value={action.label || ""} onChange={e => update("actions", details.actions.map((item, i) => i === index ? { ...item, label: e.target.value } : item))} /><select value={action.action || "request"} onChange={e => update("actions", details.actions.map((item, i) => i === index ? { ...item, action: e.target.value } : item))}><option value="request">Request access</option><option value="link">Open link</option><option value="back">Back home</option></select><input placeholder="URL (for link)" value={action.url || ""} onChange={e => update("actions", details.actions.map((item, i) => i === index ? { ...item, url: e.target.value } : item))} /><label style={{ fontSize: "0.75rem" }}><input type="checkbox" checked={action.enabled !== false} onChange={e => update("actions", details.actions.map((item, i) => i === index ? { ...item, enabled: e.target.checked } : item))} /> Enabled</label><button onClick={() => update("actions", details.actions.filter((_, i) => i !== index))}>Remove</button></div>)}<button onClick={() => update("actions", [...details.actions, { label: "Request access", action: "request", url: "", enabled: true }])} style={{ marginTop: "0.5rem" }}>+ Add action</button></div>
